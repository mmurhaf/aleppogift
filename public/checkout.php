...
function send_confirmation($order_id, $fullname, $grandTotal, $payment_method, $email, $customer_data = []) {
    ...
    try {
        ...
        error_log("📄 Starting invoice PDF generation for Order #$order_id");
        ...
        try {
            ...
            if ($result['success']) {
                ...
                error_log("✅ Invoice PDF generated successfully for Order #$order_id at: $fullPath");
            } else {
                error_log("❌ Invoice PDF generation failed for Order #$order_id: " . ($result['error'] ?? 'Unknown error'));
            }
        ...
        if (!empty($fullPath) && file_exists($fullPath)) {
            error_log("📧 Starting enhanced email send to: $email");
            
            try {
                // Use the enhanced email function with cart details, shipping info, and PDF attachment
                $status = sendEnhancedOrderEmail($email, $order_id, $fullname, $customer_data, $fullPath);
                if (!$status) {
                    error_log("❌ Failed to send enhanced email for Order #$order_id to $email.");
                    // Fallback to simple email
                    error_log("🔄 Attempting fallback to simple email...");
                    $status = sendOrderEmail($email, $order_id, $fullname);
                } else {
                    error_log("✅ Enhanced email sent successfully for Order #$order_id to $email.");
                }
            } catch (Exception $e) {
                error_log("❌ Enhanced email sending error for Order #$order_id: " . $e->getMessage());
                ...
                try {
                    ...
                    error_log("❌ Fallback email also failed: " . $fallback_e->getMessage());
                }
            } catch (Error $e) {
                error_log("💥 Enhanced email sending fatal error for Order #$order_id: " . $e->getMessage());
            ...
            try {
                // Send enhanced email without PDF attachment
                $status = sendEnhancedOrderEmail($email, $order_id, $fullname, $customer_data, null);
                if (!$status) {
                    error_log("❌ Failed to send enhanced email (no PDF) for Order #$order_id to $email.");
                    // Fallback to simple email
                    error_log("🔄 Attempting fallback to simple email...");
                    $status = sendOrderEmail($email, $order_id, $fullname);
                } else {
                    error_log("✅ Enhanced email (no PDF) sent successfully for Order #$order_id to $email.");
                }
            } catch (Exception $e) {
                error_log("❌ Enhanced email (no PDF) error for Order #$order_id: " . $e->getMessage());
                // Fallback to simple email
                error_log("🔄 Attempting fallback to simple email...");
                try {
                    $status = sendOrderEmail($email, $order_id, $fullname);
