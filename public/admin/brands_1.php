...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add_brand'])) {
    ...
    if (isset($_FILES['brand_logo']) && $_FILES['brand_logo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!in_array($file_ext, $allowed_extensions)) {
            $message = "Error: Only JPG, JPEG, PNG, GIF, WebP, and SVG files are allowed.";
    ...
    if (empty($message) || strpos($message, 'Error:') === false) {
        $db->query("INSERT INTO brands (name_ar, name_en, logo, status) VALUES (:name_ar, :name_en, :logo, 1)", [
            'name_ar' => $name_ar,
            'name_en' => $name_en,
            'logo' => $logo_filename
        ]);
...
<body>
    <div class="container-fluid py-4">
        ...
        <!-- Message Alert -->
        <?php if($message): ?>
        <div class="message-alert alert <?php echo strpos($message, 'Error:') === 0 ? 'alert-danger' : 'alert-success'; ?> alert-dismissible fade show" role="alert">
            ...
            <form method="post" enctype="multipart/form-data">
                <div class="row g-3">
                    ...
                    <div class="col-md-12">
                        ...
                        <input type="file" class="form-control" id="brand_logo" name="brand_logo" accept="image/*" onchange="previewLogo(this)">
                        <div class="form-text">Upload a high-quality logo (PNG, JPG, SVG, WebP, GIF - Max: 5MB)</div>
                        
                        <!-- Logo Preview -->
                        <div id="logoPreview" style="display: none; margin-top: 10px;">
                            <img id="previewImg" style="max-width: 100px; max-height: 100px; border-radius: 4px; border: 1px solid #ddd;" alt="Logo preview">
                            <div class="text-muted mt-1">Logo preview</div>
                        </div>
                    </div>
                    <div class="col-12">
                ...
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <?php if (!empty($brand['logo']) && file_exists('../uploads/brands/'.$brand['logo'])): ?>
