<?php
// File: admin/regenerate_invoice_no_auth.php - Testing version without admin authentication
...
try {
    ...
    // Skip admin login requirement for testing
    // require_admin_login();
    ...
    echo "<!-- Debug: Error in includes: " . $e->getMessage() . " -->\n";
...
elseif (isset($_GET['id'])) {
    echo "<!-- Debug: GET request with ID: " . $_GET['id'] . " -->\n";
    $order_id = intval($_GET['id']);
    
    try {
        $order = $db->query("SELECT * FROM orders WHERE id = ?", [$order_id])->fetch(PDO::FETCH_ASSOC);
        echo "<!-- Debug: Order query executed -->\n";
        
        if ($order) {
            echo "<!-- Debug: Order found -->\n";
            $customer = $db->query("SELECT * FROM customers WHERE id = ?", [$order['customer_id']])->fetch(PDO::FETCH_ASSOC);
            echo "<!-- Debug: Customer query executed -->\n";
            ...
            $order_items = $db->query("
                ...
                WHERE oi.order_id = ?
            ", [$order_id])->fetchAll(PDO::FETCH_ASSOC);
            echo "<!-- Debug: Order items query executed -->\n";
        } else {
            echo "<!-- Debug: Order not found -->\n";
            $message = "Order #$order_id not found.";
        }
    } catch (Exception $e) {
...
echo "<!-- Debug: About to render HTML -->\n";
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Regenerate Invoice - AleppoGift Admin (Test)</title>
...
<body>
    ...
    <div class="container-fluid py-4">
        <div class="alert alert-warning">
            <strong>DEBUG MODE:</strong> This is a test version without authentication
        </div>
        
        <!-- Header -->
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="h3 mb-0 text-gray-800">Regenerate Invoice (Test)</h1>
