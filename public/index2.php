...
if (isset($_GET['page']) && $_GET['page'] === 'product_details' && isset($_GET['code'])) {
    // Map old "code" param to new "search"
...
if ($search !== '') {
    if (is_numeric($search)) {
        // Strict: only match by exact product ID/code
        $where[] = "p.id = :product_id";
        $params['product_id'] = $search;
    } else {
        // Match by product name (English or Arabic)
...
//     $product['name_ar'] = fixArabicEncoding($product['name_ar']);
//     $product['description_ar'] = fixArabicEncoding($product['description_ar']);
// }
        ...
        <div class="col">
            <div class="card h-100 product-card">
                ...
                <div class="card-body d-flex flex-column">
                    ...
                    <h5 class="card-title product-name">  <?= htmlspecialchars($p['name_ar']); ?></h5>
                    <!-- Product Price -->
                    <p class="product-price price my-2">
                        ...
                        $<?= number_format($p['price']/3.68, 2); ?>
                         &nbsp;
                        KWD <?= number_format($p['price']*0.083, 2); ?>
